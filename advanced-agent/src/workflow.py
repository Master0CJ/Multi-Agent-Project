...
class Workflow:
    ...
    def _extract_tools_step(self, state: ResearchState) -> Dict[str, Any]:
        ...
        for result in search_results.web:
            # Fix: Access url from metadata if available, otherwise use empty string
            url = (
                getattr(result.metadata, "url", "")
            ...
            if url:
                ...
                if scraped and hasattr(scraped, "markdown"):
                    all_content += (
                        scraped.markdown[:1500] + "\n\n"
                    )  # Fix: use += instead of +

        # If we couldn't get content from scraping, use the search results directly
        if not all_content:
            for result in search_results.web:
        ...
        try:
            ...
            print(f"Error in tool extraction: {e}")
    ...
    def _research_step(self, state: ResearchState) -> Dict[str, Any]:
        ...
        if not extracted_tools:
            ...
            for result in search_results.web:
                if (
                    ...
                    and result.metadata
                    and hasattr(result.metadata, "title")
                ):
                    tool_names.append(result.metadata.title)
                elif hasattr(result, "title"):
                    tool_names.append(result.title)
                else:
                    tool_names.append("Unknown Tool")
        else:
        ...
        for tool_name in tool_names:
            ...
            if tool_search_results and tool_search_results.web:
                ...
                # Fix: Access url from metadata if available
                url = (
                    getattr(result.metadata, "url", "")
                    if hasattr(result, "metadata") and result.metadata
                    else ""
                ...
                # Get description from markdown if available
                description = (
